...
public class ChartGeneration {
	...
	//Tweet #1
	public static void languageUses(Map <String, Long> sortedMap, Integer totalSize) throws FontFormatException, IOException {
		initializeFont();
		...
		JFreeChart chart = ChartFactory.createBarChart("Breakdown of the last " + totalSize + " Tweets by " + AnalyzeLang.nickOfAPerson + " by language posted", "Language", "Amount of Tweets", dataset);
		...
		try {
			String output = "In the last " + totalSize + " Tweets by " + AnalyzeLang.nickOfAPerson + ", the most popular language";
			int defaultSize = 3;
			if (sortedMap.size()<3) defaultSize=sortedMap.size();
			if (defaultSize>1) output+="s were ";
			else output+=" was ";
			for (int i=0; i<defaultSize; i++) {
				output+= mostCommon[i] + " with " + mostCommonVals[i] + " Tweets";
				if (i==defaultSize-3) output+=", ";
				else if (i==defaultSize-2) output+=" and ";
				else if (i==defaultSize-1) output+=".";
			}
	...
	//Tweet #2
	public static void charactersInTweets(Integer[] totals, List<Integer[]> DividedIntoFourSections) throws FontFormatException, IOException {
		initializeFont();
		...
		for (Integer[] entry: DividedIntoFourSections) {
			for (int i=0; i<5; i++) data[i][ID] = entry[i];
			ID++;
		}
		for (int i=0; i<ID; i++) {
		...
		for (int i=0; i<5; i++) renderer.setSeriesPaint(i, colors[i]);
		renderer.setDrawBarOutline(true);
		renderer.setItemMargin(0);
		try {
			saveAndPost(chart, AnalyzeLang.nickOfAPerson + " in his last " + ID + " Tweets wrote " + totals[0] + " capital letters, " + totals[1] + " lowercase letters, " + totals[2] + " numbers, " + totals[3] + " spaces and " + totals[4] + " other characters.");
		} catch (TwitterException e1) {}
		charactersInTweetsBreakdown(colors, totals);
		
	}
	//Tweet #3
	public static void charactersInTweetsBreakdown(Color[] colors, Integer[] totals) throws FontFormatException, IOException {
        ...
        for (int i=0; i<5; i++) total+=totals[i];
        for (int i=0; i<5; i++) totalPercentages[i]=(double) totals[i]*100/(double) total;
                   
        JFreeChart mentions = ChartFactory.createPieChart("Percentage breakdown of the total usage of " + Integer.toString(total) + " different signs in Tweets",dataset,true,true,false);                
        ...
        try {
			saveAndPost(mentions, "In percents, " + 
					String.format("%.2f", totalPercentages[0]) + "% was taken by uppercase characters, " +
					String.format("%.2f", totalPercentages[1]) + "% by lowercase characters, " +
					String.format("%.2f", totalPercentages[2]) + "% by numbers, " +
					String.format("%.2f", totalPercentages[3]) + "% by spaces and " +
	...
	public static void mentionPieChart(Map<String, Integer> sortedMentions) throws FontFormatException, IOException {
        ...
        JFreeChart mentions = ChartFactory.createPieChart("Most common mentions in the last Tweets",sortedMentionDataSet,true,true,false);                
        PiePlot plot = (PiePlot) mentions.getPlot();
        ...
        for (int i=0; i<11; i++) colors[i] = new Color((int) (96-(96*i*0.07)), (int) (61-(61*i*0.07)), (int) (255-(255*i*0.07)));
